...
const App: React.FC = () => {
  ...
  const getOppOptimalVolume = useCallback((opp: ArbitrageOpportunity): number => {
    ...
    return net > 0.001 ? optimalV : 0; // Exigir solo +$0.001 de beneficio neto positivo (transacción netamente positiva)
    ...
    try {
      ...
      if (isAutopilot && !executing) {
        ...
        if (profitableOpp && optimalV > 0) {
          ...
          addLog(`AUTOPILOT: Analizados ${finalOpportunities.length} inbalances (${topOpp.symbol} +${topOpp.imbalancePercentage.toFixed(2)}%). Tras restar comisiones DEX (0.6%), slippage (0.2%) y gas, la ganancia neta estimada queda por debajo de +$0.001 USD. Esperando desbalance mayor...`, "info");
        }
      }
