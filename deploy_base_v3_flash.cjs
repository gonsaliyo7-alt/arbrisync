/**
 * Deploy ArbiSyncExecutor (Uniswap V3 Flash Swap edition) to Base Network
 */
...
async function main() {
  ...
  console.log('🚀 Desplegando ArbiSyncExecutor (Uniswap V3 Flash Swap Edition) en Base...');
  ...
  console.log(`   Balance en Base: ${balance.toString()} wei (${Number(balance) / 1e18} ETH)`);

  if (balance === 0n) {
    console.error('❌ No tienes ETH en Base.');
    process.exit(1);
  }

  const factory = new ContractFactory(ABI, BYTECODE, wallet);
  
  console.log('\n📦 Enviando transacción de despliegue...');
  ...
  console.log(`\n✅ ¡CONTRATO UNISWAP V3 FLASH DESPLEGADO CON ÉXITO!`);
